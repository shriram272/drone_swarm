...
class TUMParser:
    def _init_(self, input_folder):
        self.depth_model = DepthAnything.from_pretrained('LiheYoung/depth_anything_vits14')
...
class EuRoCParser:
    def _init_(self, input_folder, start_idx=0):
        self.input_folder = input_folder
...
class BaseDataset(torch.utils.data.Dataset):
    def _init_(self, args, path, config):
        self.args = args
        self.path = path
        self.config = config
        self.device = "cuda:0"
...
class MicrosoftCam(BaseDataset):
    def __init__(self, args, path, config):
        super().__init__(args, path, config)
        self.h, self.w = 480, 640
        self.live_vid = cv2.VideoCapture(0)
        self.fx = 642.951262
        self.fy = 638.070350
        self.cx = 303.182956
        self.cy = 226.158235
        self.width = int(self.live_vid.get(cv2.CAP_PROP_FRAME_WIDTH))
        self.height = int(self.live_vid.get(cv2.CAP_PROP_FRAME_HEIGHT))
        self.fovx = focal2fov(self.fx, self.width)
        self.fovy = focal2fov(self.fy, self.height)
        self.K = np.array(
            [[self.fx, 0.0, self.cx], [0.0, self.fy, self.cy], [0.0, 0.0, 1.0]]
        ...
        self.disorted = True
        self.dist_coeffs = np.array([-0.056876, 0.697706, -0.005434, -0.010707, -2.190644])
        self.map1x, self.map1y = cv2.initUndistortRectifyMap(
            self.K, self.dist_coeffs, np.eye(3), self.K, (self.w, self.h), cv2.CV_32FC1
        )

        # ROS specific initialization
        self.node = rclpy.create_node('microsoft_cam_node')
        self.bridge = CvBridge()
        self.image_pub = self.node.create_publisher(ROSImage, 'camera/image', 10)
        self.spin_thread = threading.Thread(target=self.spin)
        self.spin_thread.start()

    def spin(self):
        rate = self.node.create_rate(30)  # 30 Hz
        while rclpy.ok():
            self.publish_image()
            rate.sleep()

    def publish_image(self):
        ret1, rgb_frame = self.live_vid.read()
        if not ret1:
            return
...
class ROSDataset(BaseDataset):
    def _init_(self, args, path, config):
        ...
        while self._check_all_parameters_() or not self.image_received:
        ...
        if not self._check_all_parameters_() and self.config["ROS_topics"]["camera_info_topic"] != 'None':
    ...
    def _check_all_parameters_(self):
...
def load_dataset(args, path, config):
    if config["Dataset"]["type"] == "microsoft":
        return MicrosoftCam(args, path, config)
    elif config["Dataset"]["type"] == "turtle":
        return TurtleCam(args, path, config)
    else:
