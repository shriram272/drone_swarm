...
#         self.motor_speed_pub = rospy.Publisher("/crazyflie2_2/command/motor_speed", Actuators, queue_size=10)

#         # subscribe to Odometry topic
#         self.odom_sub = rospy.Subscriber("/crazyflie2_2/ground_truth/odometry", Odometry, self.odom_callback)
...
#         if self.t <= 5:  # P0 (1,0,0) to P1 (1,0,1)
#             coeff_x = self.traj_coeff(0, 5, 1, 1)
#             coeff_y = self.traj_coeff(0, 5, 0, 0)
#             coeff_z = self.traj_coeff(0, 5, 0, 1)

#         elif self.t <= 20:  # P1 (1,0,1) to P2 (2,0,1)
#             coeff_x = self.traj_coeff(5, 20, 1, 2)
#             coeff_y = self.traj_coeff(5, 20, 0, 0)
#             coeff_z = self.traj_coeff(5, 20, 1, 1)

#         elif self.t <= 35:  # P2 (2,0,1) to P3 (2,1,1)
#             coeff_x = self.traj_coeff(20, 35, 2, 2)
#             coeff_y = self.traj_coeff(20, 35, 0, 1)
#             coeff_z = self.traj_coeff(20, 35, 1, 1)

#         elif self.t <= 50:  # P3 (2,1,1) to P4 (1,1,1)
#             coeff_x = self.traj_coeff(35, 50, 2, 1)
#             coeff_y = self.traj_coeff(35, 50, 1, 1)
#             coeff_z = self.traj_coeff(35, 50, 1, 1)

#         elif self.t <= 65:  # P4 (1,1,1) to P5 (1,0,1)
#             coeff_x = self.traj_coeff(50, 65, 1, 1)
...
#     rospy.init_node("quadrotor_control2")
...
class Quadrotor():
    ...
    def traj_gen(self):
        ...
        height = 1.0  # Height change
        angular_speed = 0.1  # Angular speed (radians per second)

        # Calculate the desired positions
        xd = 2 + radius * np.cos(angular_speed * self.t)
        yd = radius * np.sin(angular_speed * self.t)
        zd = height * (self.t / 65.0)  # Assuming the total duration is 65 seconds

        # Calculate the desired velocities
        xd_dot = -radius * angular_speed * np.sin(angular_speed * self.t)
        yd_dot = radius * angular_speed * np.cos(angular_speed * self.t)
        zd_dot = height / 65.0

        # Calculate the desired accelerations
        xd_ddot = -radius * angular_speed**2 * np.cos(angular_speed * self.t)
        yd_ddot = -radius * angular_speed**2 * np.sin(angular_speed * self.t)
        zd_ddot = 0
